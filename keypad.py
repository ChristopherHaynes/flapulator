...
class Keypad:
    ...
    def read_keypad(self):
        ...
        for row_pin in self.row_pins:
            row_pin.value(0)  # Set column pin to LOW
            for i, col_pin in enumerate(self.column_pins):
                if not col_pin.value():  # If row pin reads LOW
                    key_pressed = self.keys[self.row_pins.index(row_pin)][i]
                    row_pin.value(1)  # Set column pin back to HIGH
                    return key_pressed
            row_pin.value(1)  # Set column pin back to HIGH
